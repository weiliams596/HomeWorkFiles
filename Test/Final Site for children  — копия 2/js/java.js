function showMessage() {
  alert("Сәлем! Роботтар мен IT әлеміне қош келдің!");
}
...
function animation() {
  let isLeft = false;
  if (window.location.pathname.split('/')[window.location.pathname.split('/').length - 1] !== 'game.html') {
    return;
  }
  /**
   * Robot
...
if (play_btn)
  document.getElementById('play-btn').addEventListener('click', () => {
    ...
    gamesList.style.top = '80%'; // Тізімді астына түсіру
  });
...
function checkGuess() {
  ...
  if (guess === randomNumber) {
    result.textContent = "Құттықтаймыз! Сіз дұрыс таптыңыз!";
  } else if (guess < randomNumber) {
    result.textContent = "Үлкенірек санды енгізіңіз.";
  } else {
    result.textContent = "Кішірек санды енгізіңіз.";
  }
}
...
const quizData = [
  {
    ...
    options: ["Гипермәтін белгілеу тілі", "Бағдарламалау тілі", "Стильдер тілі"],
    answer: "Гипермәтін белгілеу тілі"
  },
  {
    question: "JavaScript не үшін қолданылады?",
    options: ["Стильдер қосу", "Интерактивтілік қосу", "Деректер базасын басқару"],
    answer: "Интерактивтілік қосу"
  }
];
...
function submitQuiz() {
  ...
  document.getElementById('quiz-result').textContent = `Сіз ${score} дұрыс жауап бердіңіз!`;
}
...
document.querySelectorAll('.block').forEach(block => {
  block.addEventListener('dragstart', (e) => {
    e.dataTransfer.setData('text/plain', block.getAttribute('data-command'));
  });
});
...
if (workspace)
  workspace.addEventListener('drop', (e) => {
    ...
    newBlock.textContent = command;
    workspace.appendChild(newBlock);
  });

function runProgram() {
  const commands = Array.from(workspace.children).map(block => block.textContent);
  let x = 0, y = 0;
  commands.forEach(command => {
    if (command === 'Алға жүру') y += 50;
    if (command === 'Оңға бұрылу') x += 50;
    if (command === 'Солға бұрылу') x -= 50;
  });
...
// 4 ойын
const tekseru = document.querySelector('.tekseru');
let x = 0, y = 0;
...
document.addEventListener('keydown', (e) => {
  ...
  tekseru.style.transform = `translate(${x}px, ${y}px)`;

  // Шығу нүктесіне жеткендігін тексеру
  if (x >= 250 && y >= 250) {
    alert("Құттықтаймыз! Сіз лабиринттен шықтыңыз!");
  }
});


//5 jqsy
const workspaces = document.querySelector('.code-workspace');

document.querySelectorAll('.code-piece').forEach(piece => {
...
if (workspace)
  workspace.addEventListener('drop', (e) => {
    ...
    newPiece.className = 'code-piece';
    newPiece.textContent = code;
    workspace.appendChild(newPiece);
  });

function checkCode() {
  const correctOrder = ['<html>', '<head>', '<title>Менің сайтым</title>']; // Дұрыс рет
  const userOrder = Array.from(workspace.children).map(piece => piece.textContent);
  if (JSON.stringify(userOrder) === JSON.stringify(correctOrder)) {
    alert("Құттықтаймыз! Сіз кодты дұрыс құрастырдыңыз!");
  } else {
    alert("Қате! Кодты қайтадан құрастырыңыз.");
  }
}
...
function moveRobot() {
  a += 50; // Мысалы, роботты оңға жылдыру
  robot.style.transform = `translate(${x}px, ${y}px)`;

  // Мақсатқа жеткендігін тексеру
  if (a >= 200 && b >= 200) {
    alert("Робот мақсатқа жетті!");
  }
}
...
function checkCode() {
  ...
  if (userCode === 'moveForward()') {
    alert("Дұрыс! Сіз кодты дұрыс айналдырдыңыз!");
  } else {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!videoButton || !lessonList) {
    console.error("Элемент табылмады!");
    return;
  }

  videoButton.addEventListener('click', function () {
    console.log("Батырма басылды!"); // Консольде тексереміз
